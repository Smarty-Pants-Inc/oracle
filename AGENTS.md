...
- Live smoke tests: OpenAI/ChatGPT live tests are opt-in. Do not send prompts to ChatGPT browser mode without explicit approval for that live run. For API smoke tests, run `ORACLE_LIVE_TEST=1 pnpm vitest run tests/live/openai-live.test.ts` with a real `OPENAI_API_KEY` when you need the background path; gpt-5-pro can take ~10 minutes.
...
- Browser prompt submission must prove ChatGPT accepted the prompt from semantic DOM state, not from a click/keypress alone. Maintain multiple independent signals (new turn, stop control, thinking/status/progress/shimmer, composer cleared/disabled, assistant turn) plus explicit blockers (login, Cloudflare, account security, workspace permission). When ChatGPT changes the DOM, extend the semantic probe and keep structured `BrowserAutomationError.details` evidence instead of adding one-off sleeps.
- Project/thread-scoped ChatGPT URLs must fail closed. Do not fall back from `/g/.../project` or `/c/...` to root `https://chatgpt.com/`; redirects to root should surface `scope-mismatch`/`scope_mismatch` diagnostics so agents can repair the configured project or login instead of polluting main chat history.
- Project-scoped ChatGPT browser smokes must also prove cleanup, not just scope. Use `--browser-archive auto` for one-shot project smokes and verify `browser.archive.archived: true` in session metadata after any created `/c/...` conversation; if the run fails before conversation creation, report the structured blocker. If cleanup cannot be verified, stop and preserve reattach evidence instead of running another smoke. Do not clean up already-created ChatGPT test conversations without explicit approval for the conversation ids/URLs.
