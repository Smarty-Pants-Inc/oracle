...
if (!ENABLE_LIVE || !LIVE_API_KEY) {
  ...
  describe("OpenAI live smoke tests", () => {
    ...
    test(
      "gpt-5.6-sol-pro background flow eventually completes",
      async () => {
        const result = await runOracle(
          {
            prompt: 'Reply with "live 5.6 pro smoke test" on a single line.',
        ...
        expect(text.toLowerCase()).toContain("live 5.6 pro smoke test");
        expect(result.response.status ?? "completed").toBe("completed");
      },
      30 * 60 * 1000,
    );
