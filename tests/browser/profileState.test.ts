...
describe("profileState", () => {
  ...
  test("preserves an unreadable profile lock and fails closed", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "oracle-profile-"));
    try {
      const lockPath = path.join(dir, "oracle-automation.lock");
      await writeFile(lockPath, "not-json");
      await expect(
        profileState.acquireProfileRunLock(dir, { timeoutMs: 2000, pollMs: 50 }),
      ).rejects.toThrow(/unreadable.*refusing to delete/i);
      expect(existsSync(lockPath)).toBe(true);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
