...
describe("api key logging", () => {
  test("prints API model suffix + alias note for gpt-5.1-pro", async () => {
    ...
    const combined = logs.join("\n");
    expect(combined).toContain("Calling gpt-5.1-pro (API: gpt-5.6-sol)");
    expect(combined).toContain("Resolved model: gpt-5.1-pro → gpt-5.6-sol");
    expect(combined).toContain(
      "Note: `gpt-5.1-pro` is a stable CLI alias; OpenAI API uses `gpt-5.6-sol`",
    );
  ...
  test("adds API suffix to verbose key log when model is an alias", async () => {
    ...
    const combined = logs.join("\n");
    expect(combined).toContain(
      "Using apiKey option=sk-s****1234 for model gpt-5.1-pro (API: gpt-5.6-sol)",
    );
    expect(combined).not.toContain("supersecret");
  });
