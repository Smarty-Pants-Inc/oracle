...
function renderHelpFooter(program: Command, colors: HelpColors): string {
  const tips = [
    ...
    `${colors.bullet("•")} Non-preview runs spawn detached sessions (especially gpt-5.6-sol-pro API). If the CLI times out, do not re-run — reattach with ${colors.accent("oracle session <slug>")} to resume/inspect the existing run.`,
    ...
    `${colors.bullet("•")} Need hidden flags? Run ${colors.accent(`${program.name()} --debug-help`)} to list search/token/browser overrides.`,
