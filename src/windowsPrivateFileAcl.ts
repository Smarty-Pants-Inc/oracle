...
const WINDOWS_PRIVATE_FILE_PROTECTED_MARKER = "oracle.windows-private-file.v1:protected";
...
export interface WindowsPrivateFileAclRequest {
  ...
  /** Fail if the path exists and establish the exact private DACL during creation. */
...
export type WindowsFileAclCommandOptions = WindowsAclCommandOptions;
export type WindowsFileAclCommandExecutor = WindowsAclCommandExecutor;
...
export type WindowsPrivateFileInitializationAuthority = (filePath: string) => Promise<boolean>;
export type WindowsPrivateFileProtectionAuthority = (filePath: string) => Promise<void>;
export type WindowsPrivateFileVerificationAuthority = (filePath: string) => Promise<void>;
...
const WINDOWS_PRIVATE_DIRECTORY_CREATION_SCRIPT = String.raw`
...
const WINDOWS_PRIVATE_FILE_CREATION_SCRIPT = String.raw`
function New-PrivateFile([string]$FilePath, [bool]$RequirePrivateParent) {
  ...
  if ($RequirePrivateParent) { Assert-PrivateAcl (Get-PhysicalItem $ParentPath $true) $true }
...
const WINDOWS_PRIVATE_ACL_PROTECTION_SCRIPT = String.raw`
function Set-CanonicalPrivateAcl([System.IO.FileSystemInfo]$Item, [bool]$Directory) {
  $Item.SetAccessControl((New-PrivateAcl $Directory))
  $Current = Get-PhysicalItem $Item.FullName $Directory
  Assert-PrivateAcl $Current $Directory
  return $Current
}
`;
...
if ($null -ne $InitializeFilePath) {
  ...
  New-PrivateFile $InitializeFilePath $true
}
...
${WINDOWS_PRIVATE_DIRECTORY_CREATION_SCRIPT}
${WINDOWS_PRIVATE_FILE_CREATION_SCRIPT}
...
type WindowsPrivateFileOperation = "initialize" | "create" | "protect" | "verify";

function encodeWindowsPrivateFileCommand(
  filePath: string,
  operation: WindowsPrivateFileOperation,
): string {
  ...
  switch (operation) {
    case "initialize":
      capabilityScript = WINDOWS_PRIVATE_FILE_CREATION_SCRIPT;
      operationScript = String.raw`
if ([System.IO.File]::Exists($FilePath) -or [System.IO.Directory]::Exists($FilePath)) {
  ...
  [Console]::Out.Write('${WINDOWS_PRIVATE_FILE_CREATED_MARKER}')
    ...
    case "create":
      capabilityScript = WINDOWS_PRIVATE_FILE_CREATION_SCRIPT;
      operationScript = String.raw`
New-PrivateFile $FilePath $false
[Console]::Out.Write('${WINDOWS_PRIVATE_FILE_CREATED_MARKER}')`;
      break;
    case "protect":
      capabilityScript = WINDOWS_PRIVATE_ACL_PROTECTION_SCRIPT;
      operationScript = String.raw`
$Item = Get-PhysicalItem $FilePath $false
try { Assert-PrivateAcl $Item $false } catch { [void](Set-CanonicalPrivateAcl $Item $false) }
[Console]::Out.Write('${WINDOWS_PRIVATE_FILE_PROTECTED_MARKER}')`;
      break;
    case "verify":
      operationScript = String.raw`
Assert-PrivateAcl (Get-PhysicalItem $FilePath $false) $false
...
function buildWindowsPrivateFileCommand(
  filePath: string,
  operation: WindowsPrivateFileOperation,
): WindowsAclCommand {
  ...
  return {
    ...
    args: [
      ...
      "-NonInteractive",
      "-EncodedCommand",
      encodeWindowsPrivateFileCommand(filePath, operation),
    ],
    options: { timeoutMs: WINDOWS_PRIVATE_TREE_ACL_TIMEOUT_MS },
  };
}
...
export function buildWindowsPrivateFileInitializationCommand(filePath: string): WindowsAclCommand {
  return buildWindowsPrivateFileCommand(filePath, "initialize");
}

export function buildWindowsPrivateFileProtectionCommand(filePath: string): WindowsAclCommand {
  return buildWindowsPrivateFileCommand(filePath, "protect");
}

export function buildWindowsPrivateFileVerificationCommand(filePath: string): WindowsAclCommand {
  return buildWindowsPrivateFileCommand(filePath, "verify");
}

export function buildWindowsPrivateFileAclCommand(
  ...
  return buildWindowsPrivateFileCommand(
    request.filePath,
    request.createNew ? "create" : request.repair ? "protect" : "verify",
  );
}
...
export async function protectWindowsPrivateFile(
  ...
  await runWindowsPrivateAclCommand(
    buildWindowsPrivateFileProtectionCommand(filePath),
    [WINDOWS_PRIVATE_FILE_PROTECTED_MARKER],
...
export async function applyWindowsPrivateFileAcl(
  request: WindowsPrivateFileAclRequest,
  execute: WindowsFileAclCommandExecutor = executeWindowsAclCommand,
): Promise<void> {
  const marker = request.createNew
    ? WINDOWS_PRIVATE_FILE_CREATED_MARKER
    : request.repair
      ? WINDOWS_PRIVATE_FILE_PROTECTED_MARKER
      : WINDOWS_PRIVATE_FILE_VERIFIED_MARKER;
  await runWindowsPrivateAclCommand(
    buildWindowsPrivateFileAclCommand(request),
    [marker],
    request.createNew
      ? "Windows private file creation failed"
      : request.repair
