...
const consultAgentBlockerShape = z.object({
  ...
  severity: z.enum(["action_required", "retryable", "fatal", "unknown"]),
  message: z.string(),
...
export function buildConsultDryRunResolved({
  ...
  if (runOptions.model === "gpt-5.6-sol-pro" && thinkingTime === "heavy") {
    guidance.push(
      'gpt-5.6-sol-pro should normally use Pro Extended. Use model:"gpt-5.5" with browserThinkingTime:"heavy" only when you explicitly want Thinking Heavy.',
    );
  }
...
export async function runConsultTool(
  ...
  try {
    ...
    const finalMeta =
      (await sessionStore.readSession(sessionMeta.id).catch(() => null)) ?? sessionMeta;
    ...
    return {
      ...
      structuredContent: {
        ...
        status: finalMeta.status === "running" ? "error" : finalMeta.status,
        output: logTail || message,
