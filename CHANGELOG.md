...
- Remote/Bridge: make native Windows PowerShell/OpenSSH reverse tunnels avoid unsupported control-socket multiplexing, publish foreground/background bridge state only after an authenticated health proof succeeds through the remote forward, and retain background hosts plus SSH descendants in a trusted kill-on-close Job Object so rollback never uses ambient `taskkill` or an unbound PID.
- Remote/Bridge: authenticate durable transaction records with a stable owner-only key independent of the connection credential, verify their trusted filename and exact payload before recovery or cleanup, and preserve unsigned, forged, wrong-key, or swapped records for manual recovery instead of using their authority.
...
- Browser: bind deferred tab cleanup to opaque, generation-scoped live close capabilities, retaining exact same-controller retries while refusing to recreate close permission from saved DevTools, target, process, or marker metadata after capability loss. Manual and borrowed tabs remain open with retryable cleanup pending; only authenticated teardown of the exact owned temporary Chrome process may subsume target closure.
...
- OpenAI: use the logical `gpt-5.6-sol-pro` alias as the API/browser default, mapping API runs to `gpt-5.6-sol` with Pro reasoning mode and routing legacy Pro aliases to the same behavior.
...
- Remote Chrome + remote browser service: `oracle serve` launches Chrome with a loopback host default and an explicit caller-supplied modern key, never logs that key, requires the host profile to be signed in, and supports reusing an existing Chrome via `--remote-chrome <host:port>` (IPv6 with `[host]:port`), including remote attachment uploads and clearer validation errors.
