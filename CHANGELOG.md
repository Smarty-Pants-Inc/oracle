...
- Browser: keep exact conversation exports clean when ChatGPT replaces Oracle's page wrappers during hydration; pre-arm hook-specific fail-closed markers, retire legacy disabled sentinels to stop stale registrations from re-firing, remove new-document registrations before page teardown, and retain markers for unknown owners; and treat an already-removed exact registration as an idempotent cleanup success.
- Browser: bind capture and archived-recovery hooks, persisted payloads, polling, retrieval, and cleanup to a per-run versioned owner; preserve legacy fail-closed sentinels as migration fences without consuming stale same-target state or bodies during migration.
- Browser/CLI: bind wrapper request origins to the fixed user/agent account and workspace before creating or resuming a session, prove the exact named workspace row before trusting a stored OBU workspace digest, preserve that evidence through restart, reject origin conflicts at direct browser entry, and refuse transient `/c/WEB:<request-id>` URLs as conversation/export affinity.
