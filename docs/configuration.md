...
```json5
{
  ...
  model: "gpt-5.6-sol-pro", // API: gpt-5.6-sol + reasoning.mode=pro; browser: ChatGPT Pro
  search: "on", // "on" | "off"
...
- Remote browser authority resolves in trust order: explicit CLI flags, then `ORACLE_REMOTE_HOST` / `ORACLE_REMOTE_TOKEN` / `ORACLE_REMOTE_LEGACY_TOKEN` / `ORACLE_REMOTE_ALLOW_LEGACY_TEXT_PROTOCOL`, then mutable user defaults in `browser.remoteHost` / `browser.remoteToken` / `browser.remoteLegacyToken` / `browser.remoteAllowLegacyTextProtocol`. A use-scoped CLI or environment pair therefore cannot be redirected by stale or attacker-modified user config. Values are validated immediately before a selected remote transport is constructed or a bridge operation consumes them, so dormant bridge state cannot block local commands or explicit API CLI/MCP consults. Every credential used remotely must be exactly 64 lowercase hexadecimal characters (32 bytes). The immediately preceding base-generated 32-character credentials remain rejected; rotate with `oracle bridge host --token auto`, clear stale config/env values, copy the new connection artifact, and re-import it with `oracle bridge client --connect ~/bridge-connection.json` as documented in [Bridge](bridge.md#upgrade-from-the-immediately-preceding-32-hex-default). Legacy fallback remains disabled unless the boolean opt-in resolves true, and the legacy bearer must differ from the modern v3 HMAC root key.
