...
title: Overview
permalink: /
description: "Oracle bundles your prompt and files so a mythical pro agent — GPT-5.6 Sol Pro, Gemini 3 Pro, Claude Opus, and friends — can answer with real repository context. CLI, MCP, browser, and API in one tool."
---
...
```bash
# Browser path — no API key, drives ChatGPT directly (default: GPT-5.6 Sol Pro).
...
- **Choosing a model.** The [Mythical Pro Agents](mythical-pro-agents.md) lineup covers when to reach for GPT-5.6 Sol Pro vs. Gemini 3 Pro vs. Claude Opus, and what each costs.
...
The frontier models marked "Pro" — GPT-5.6 Sol Pro, Gemini 3 Pro, Claude Opus, OpenAI Deep Research — are slow, expensive, and gated behind ChatGPT Plus / Pro tiers, separate APIs, or per-token bills that scale fast. Oracle is the single entry point: one config, one session store, one set of flags. Bundle the right files, ask the right model, get a second opinion without remembering which provider charges for what.
