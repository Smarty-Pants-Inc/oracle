...
- `--model`: the same flag used for API runs is accepted. Oracle's `gpt-5.6-sol-pro` alias selects ChatGPT's current `Pro` picker in browser mode and maps to `gpt-5.6-sol` with Pro reasoning in API mode. The ChatGPT automation path also supports GPT-5.5, GPT-5.4, and GPT-5.2 variants. Legacy Pro aliases resolve to `gpt-5.6-sol-pro`.
